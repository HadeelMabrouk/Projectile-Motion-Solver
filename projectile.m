...
while toggle
    if (abs(ve-vs)<0.001 || abs(thetae-thetas)<0.001) %to exit when the start and end value are almost the same
        ...
        [t,sol]=ode45(@projectile,(0:0.01:100),y,Opt);
...
function [xp] = projectile(t,sol)
    xp=zeros(4,1);
    xp(1)=sol(2);    % dx/dt
    xp(2)=(-0.25/10)*sqrt( ((sol(2))^2) + ((sol(4))^2) )*sol(2);   %dvx/dt
    xp(3)=sol(4);     % dz/dt
    xp(4)=-9.81-(0.25/10)*sqrt( ((sol(2))^2) + ((sol(4))^2) )*sol(4); %dvz/dt
end
